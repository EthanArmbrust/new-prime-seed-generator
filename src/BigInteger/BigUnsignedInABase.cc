...
//	std::cout << "created the correct object" << std::endl;
	// Check the base.
	if (base > 36)
	...
	for (digitNum = 0; digitNum < len; digitNum++) {
		...
		if (theSymbol >= '0' && theSymbol <= '9')
			...
			std::cout << "Throwing bad symbol error" << std::endl;
			std::cout << "Offending Symbol = \"" << theSymbol << "\"" << std::endl;
			std::cout << "len = " << len << std::endl;
			std::cout << "symbolNumInString: " << symbolNumInString << std::endl;
			throw "BigUnsignedInABase(std::string, Base): Bad symbol in input.  Only 0-9, A-Z, a-z are accepted.";
		}
		if (blk[digitNum] >= base){
		  std::cout <<"Throwing big number error" << std::endl;
			throw "BigUnsignedInABase::BigUnsignedInABase(const Digit *, Index, Base): A digit is too large for the specified base";}
	}
	zapLeadingZeros();
}
